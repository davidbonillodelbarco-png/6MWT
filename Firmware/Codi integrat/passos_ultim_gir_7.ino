  ...
  void onWrite(BLECharacteristic* pCharacteristic) {
    ...
    if (valor == "go") {
      ...
      if (isRunning) {
        ...
        // Registrar el tiempo del ÚLTIMO tramo (desde el último giro hasta que se pulsó stop)
        float segundosUltimoTramo = (millis() - tiempoUltimoGiro) / 1000.0;
        if (tiemposIntervalos.length() > 0) {
          tiemposIntervalos += ",";
        }
        tiemposIntervalos += String(segundosUltimoTramo, 1);
